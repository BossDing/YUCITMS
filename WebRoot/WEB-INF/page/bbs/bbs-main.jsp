...
<script>
        function closeWindow () {
            ...
            $(document).ready(function(){
                ...
                $.post('bbs_getTodayPostsNumber.action',function(data) {
...
<body>
    <header class="head box-shdow bg-style">
        ...
        <div class="center">
            ...
            <img id="myquestion" style="cursor: pointer;" onclick="javascript:location.href='<%=basePath%>/bbs_myPosts.asp'" src="<%=basePath%>/bbsStyle/images/my_question.png" />
    ...
    <c:forEach var="entity" items="${recordList}">
        <%-- <c:if test="${!empty entity.posts}"> --%>
        <div class="content-area font-style box-shdow">
            <h3 class="block-title">${entity.name}</h3>
            <span title="more" class="operator">更多</span>
            <c:forEach var="post" items="${entity.posts}" varStatus="status">
                <div class="content" title="点击查看详情">
                    ...
                    <span onclick="openUrl('bbs_findRespData.action?id=${post.id}');" class="title-content">${post.title}</span>
                    <span class="suffix">
   ...
   <div id="page">
       ...
       <s:form action="bbs_index" name="page">
