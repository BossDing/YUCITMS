...
function checkAll(field){
	...
	for(var i=0; i<idCheckboxs.length; i++){
		...
		if(field.checked){
			...
			var date={"qgroupingId":'${qgroupingId}',"questionId":idCheckboxs[i].value,"questionType":"danxuan"};
	...
	function check(field){
		if(field.checked){
			var url="paper_addQuestionToGrouping.action";
			var date={"qgroupingId":'${qgroupingId}',"questionId":field.value,"questionType":"danxuan"};
			$.post(url,date,function(result){
				if(result=='true')
			field.checked = true;				
			});
...
<body>
    ...
    <table class="tablelist">
        ...
        <td onclick="show('${id}、${questions_DX}','${A}','${B}','${C}','${D}','${answer_DX}','${parse}');">
        <s:if test="%{questions_DX.length()>10}">
        ${fn:substring(questions_DX,0,10)}...
        </s:if>
        <s:else>
        ${questions_DX}
        </s:else>
    ...
    <s:form action="paper_getDXquestionList?qgroupingId=%{qgroupingId}" name="page">
