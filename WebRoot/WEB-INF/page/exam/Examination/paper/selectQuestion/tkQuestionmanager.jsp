...
$(document).ready(function(e) {
	...
	for(var i=0; i<idCheckboxs.length; i++){
		...
		if(field.checked){
			...
			var date={"qgroupingId":'${qgroupingId}',"questionId":idCheckboxs[i].value,"questionType":"tiankong"};
	...
	function check(field){
		if(field.checked){
			var url="paper_addQuestionToGrouping.action";
			var date={"qgroupingId":'${qgroupingId}',"questionId":field.value,"questionType":"tiankong"};
			$.post(url,date,function(result){
				if(result=='true')
			field.checked = true;				
			});
	...
	function show(content,answer){
		layer.open({
			  type: 1,
			  skin: 'layui-layer-rim', //加上边框
			  area: ['420px', '240px'], //宽高
			  content: content+'<hr />答案：'+answer
			});	
	}
...
<body>
    ...
    <table class="tablelist">
        ...
        <td onclick="show('${id}、${questions_TK}','${answer_TK}');">
        <s:if test="%{questions_TK.length()>10}">
        ${fn:substring(questions_TK,0,10)}...
        </s:if>
        <s:else>
        ${questions_TK}
        </s:else>
    ...
    <s:form action="paper_getTKquestionList?qgroupingId=%{qgroupingId}" name="page">
