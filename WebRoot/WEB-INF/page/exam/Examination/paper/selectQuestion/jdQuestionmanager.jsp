...
function checkAll(field){
	...
	for(var i=0; i<idCheckboxs.length; i++){
		...
		if(field.checked){
			...
			var date={"qgroupingId":'${qgroupingId}',"questionId":idCheckboxs[i].value,"questionType":"jianda"};
	...
	function check(field){
		if(field.checked){
			var url="paper_addQuestionToGrouping.action";
			var date={"qgroupingId":'${qgroupingId}',"questionId":field.value,"questionType":"jianda"};
			$.post(url,date,function(result){
				if(result=='true')
			field.checked = true;				
			});
...
<body>
    ...
    <table class="tablelist">
        ...
        <td onclick="show('${id}、${questions_JD}','${answer_JD}','${parse}');">
        <s:if test="%{questions_JD.length()>10}">
        ${fn:substring(questions_JD,0,10)}...
        </s:if>
        <s:else>
        ${questions_JD}
        </s:else>
    ...
    <s:form action="paper_getJDquestionList?qgroupingId=%{qgroupingId}" name="page">
