...
function checkAll(field){
	...
	for(var i=0; i<idCheckboxs.length; i++){
		...
		if(field.checked){
			...
			var date={"qgroupingId":'${qgroupingId}',"questionId":idCheckboxs[i].value,"questionType":"panduan"};
	...
	function check(field){
		if(field.checked){
			var url="paper_addQuestionToGrouping.action";
			var date={"qgroupingId":'${qgroupingId}',"questionId":field.value,"questionType":"panduan"};
			$.post(url,date,function(result){
				if(result=='true')
			field.checked = true;				
			});
	...
	function show(content,answer,parse){
		layer.open({
			  type: 1,
			  skin: 'layui-layer-rim', //加上边框
			  area: ['420px', '240px'], //宽高
			  content: content+'<hr />答案：'+answer+'<hr />解析：'+parse
			});	
	}
...
<body>
    ...
    <table class="tablelist">
        ...
        <td onclick="show('${id}、${questions_PD}','${answer_PD==1?'对':'错'}','${parse}');">
        <s:if test="%{questions_PD.length()>10}">
        ${fn:substring(questions_PD,0,10)}...
        </s:if>
        <s:else>
        ${questions_PD}
        </s:else>
    ...
    <s:form action="paper_getPDquestionList?qgroupingId=%{qgroupingId}" name="page">
