...
$(function() {
    ...
    $(".section_th").click(
            function() {
                $(this).parent().parent().children("tr").not(":first").toggle(
                        'fast');
                $(this).children("span").eq(1).html(
                        "0" == $(this).children("span").eq(1).html() ? "1"
                                : "0");
            });

    // 移除试题详情
    $("#div_question_list").bind("mouseleave", function() {
        $("#question_detail").fadeOut();
    });

    $('#question_detail_left').bind("mouseleave", function() {
        $("#question_detail_left").fadeOut();
    });
});
...
function list(page) {
    ...
    $
            .getJSON(
                    "paper_getQuestions.asp",
                    {
                        page : page,
                        rand_dbid : s_dbid,
                        rand_qtype : s_qtype,
                        rand_cname : encodeURI(s_keywords)
                    },
                    function(data) {
                        $("#questionlist").empty();
                        $("#questionlist")
                                .append(
                                        '<tr><th width="30">编号</th><th width="30">类型</th><th>题干</th><th>操作</th></tr>');
                        $(data.datalist)
                                .each(
                                        function(i) {
                                            ...
                                            html += '<td>' + this.ID + '</td>';
                                            html += '<td>'
                                                    + qTypeName(this.QTYPE)
                                                    + '</td>';
                                            html += '<td>' + content + '</td>';
                                            html += '<td>';
                                            html += '<a href="javascript:;" onclick="addQuestion('
                                                    + this.ID
                                                    + ',this,'
                                                    + this.QTYPE
                                                    + ')" title="添加到试卷"><img src="images/ico_add.png" border="0" align="absmiddle" /></a> ';
                                            html += '</td>';
                                            html += '</tr>';
                        ...
                        // 绑定试题详情事件
                        /*
                         * $(".tr_of_question").mouseover( function(ev) { var
                         * qid = $(this) .children("td:first-child").html(); var
                         * offset = $(this).offset(); var the_top = offset.top +
                         * 28; $("#question_detail").attr("style",
                         * "left:600px"); $("#question_detail").attr("style",
                         * "top:" + the_top + "px");
...
function addQuestion(qid, obj, qtype) {
    ...
    // layer.confirm(addQuestionConfirm(qid, qtype), function() {
    // addQuestionCallback(v, f);
    // });
    $.prompt(addQuestionConfirm(qid, qtype), {
        buttons : {
            ok : true,
            cancel : false
        },
        submit : addQuestionCallback
    });
}
...
function addQuestionConfirm(qid, qtype) {
    ...
    var html = '<script src="js/textcontral.js" type="text/javascript"></script>';
    var sections = '<select name="sectionid" id="sectionid">';
    $(SECTION_LIST).each(
            function() {
                sections += '<option value="' + this.ID + '">'
                        + this.SECTION_NAME + '</option>';
            });
    sections += "</select>";
    html += '请选择需要添加到的章节？<br/>';
    html += '目标章节：' + sections + '<br/>';
    html += '本题分值：<input maxlength="2" type="text" class="txtfree" name="cqscore" id="cqscore" value="0" size="5" style="margin-top:5px" /> 分<br/>';
...
function addQuestionCallback(v, m, f) {
    ...
    if (true == v || "true" == v) {
        if (document.getElementById("QUESTION_" + qid)) {
            ...
            html += ' <td>';
            html += ' 编号：[' + qid + '] &nbsp; ';
            html += ' 分值：<input type="text" maxlength="2" class="validate[required,custom[onlyNumber]] txtfree" size="2" value="'
                    + score + '" name="qscore" />&nbsp;';
            html += ' <a href="javascript:;" onclick="removeQuestion(\'' + qid
                    + '\');"><u>移除</u></a> |';
            html += ' <a href="javascript:;" onclick="moveQuestion(\'' + qid
                    + '\',\'up\');"><u>上移</u></a> |';
            html += ' <a href="javascript:;" onclick="moveQuestion(\'' + qid
                    + '\',\'down\');"><u>下移</u></a> |';
            html += ' <a href="javascript:;" onclick="viewQuestionLeft(' + qid
                    + ',this)"><u>详情</u></a>';
            html += ' <br/>';
...
function viewQuestion(obj, qid) {
    ...
    $.getJSON("paper!getQuestionDetail.htm", {
        id : qid
    }, function(data) {
        ...
        if (qtype == "1") {// 选择
            ...
            $(data.datalist.OPTIONS).each(function() {
                html += this.OID + '：' + this.SOPTION + '<br/>';
            });
            html += '<div><b>答案：</b>' + data.datalist.SKEY + '</div>';

        } else if (qtype == "3") {// 判断
            html += '<div><b>答案：</b>'
                    + ((data.datalist.SKEY == "YES") ? "正确" : "错误") + '</div>';
