    ...
    $(".qks")
            .each(
                    function() {
                        ...
                        var did = $(this).attr("qid");
                        var html = '';
                        html += '<a href="javascript:;" style="color:#f00" onmousemove="buildSetter('
                                + qscore + ',this,' + did + ');">设置得分</a>';
                        $(this).append("<br/>" + html);
                    });
...
buildSetter = function(qscore, obj, did) {
    ...
    for (var i = 0; i <= eval(qscore); i++) {
        btns += '<a href="javascript:;" onclick="setScore(' + i + ',' + did
                + ');" class="lnk_score">' + i + '</a> ';
    }
    ...
    $.ajax({
        ...
        success : function(msg) {
            if ("1" == msg || 1 == msg || 2 == msg || "2" == msg) {
                alert("操作成功");
                // 在页面上设置改变后的分值
                var old_this_val = $(".qks[qid=" + _did + "]").children(
                        "span.label:first").html();
                var old_total_score = $("#u_total_score").html();

                var n_this_val = _score;
                var n_total_score = eval(old_total_score - old_this_val
                        + n_this_val);
                $("#u_total_score").html(n_total_score);
                $(".qks[qid=" + _did + "]").children("span.label:first").html(
                        "" + n_this_val);
            } else {
                alert("操作失败");
            }
