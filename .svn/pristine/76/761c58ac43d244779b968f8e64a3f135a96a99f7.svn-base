...
<body>
   ...
   <form action="difficulty_addDifficultyGuidance.action?courseId=${courseId}" method="post" enctype="multipart/form-data" onsubmit="javascript:if(!confirm('确实要操作吗？'))return false;">
    <ul class="forminfo">
    <li><label>重难点</label><input name="diffName" type="text" class="dfinput" /><i>标题不能超过30个字符</i></li>
    <li><label>描述</label><textarea name="diffScheme" cols="" rows="" class="textinput"></textarea></li>
