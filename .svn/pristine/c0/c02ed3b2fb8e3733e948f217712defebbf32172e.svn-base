...
<body>
    ...
    <form id="form1" action="excellentTeacher_addExcellentTeacher.action?id=${id}&courseId=${courseId}" method="post" enctype="multipart/form-data" onsubmit="javascript:if(!confirm('确定操作吗？'))return false;">
    <ul class="forminfo">
    <li><label>教师姓名</label><input name="teacherName" type="text" class="dfinput" /><i>标题不能超过30个字符</i></li>
    <li><label>职称</label><input name="jobTitle" type="text" class="dfinput" /><i>标题不能超过30个字符</i></li>
    <li><label>简介</label><textarea name="content" cols="" rows="" class="textinput"></textarea></li>
