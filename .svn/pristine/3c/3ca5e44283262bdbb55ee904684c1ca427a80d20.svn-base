...
<body>
    ...
    <td><a href="course_getCourse.action?id=${id}" onclick="load();">${name}</a><p>发布时间：<s:date name="createDate" format="yyyy-MM-dd HH:mm:ss"/></p></td>
    <td>
    ...
    <a href="course_deleteCourse.action?id=${id }" class="button button-blue button-tiny" onclick="javascript:if(!confirm('删除后可在回收站恢复，确实要删除吗？'))return false;">删除</a>
    <s:if test="%{zhuxian==true}">
    <a href="course_cancelZhuXian.action?id=${id }" class="button button-blue button-tiny" onclick="javascript:if(!confirm('此操作会占用大量性能，应减少频繁操作。确定要操作吗？'))return false;"><label style="color: red;">取消主显</label></a>
    </s:if>
    <s:else>
    <a href="course_setZhuXian.action?id=${id }" class="button button-blue button-tiny" onclick="javascript:if(!confirm('此操作会占用大量性能，应减少频繁操作。操作后，精品课程主页默认显示当前课程。确定要操作吗？'))return false;">设置为主显</a>
    </s:else> 
    </td>
    </tr>
    </s:iterator>
